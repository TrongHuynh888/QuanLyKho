...
type ReportTab = "dashboard" | "inventory" | "balance" | "movement" | "expiry" | "supplier";
...
/**
 * Component hiển thị giao diện báo cáo và biểu đồ phân tích
 * Cung cấp cái nhìn tổng quan và các báo cáo chi tiết về: tồn kho, di chuyển, hạn sử dụng, nhà cung cấp.
 *
 * @returns {JSX.Element} Giao diện Báo cáo
 */
export default function ReportsView() {
  ...
  // Tải dữ liệu hoạt động cho biểu đồ
  useEffect(() => {
    (async () => {
  ...
  /**
   * Tính toán và tổng hợp dữ liệu cho biểu đồ theo các mốc thời gian
   * @param {any[]} activities Mảng các hoạt động xuất nhập kho
   * @param {"week" | "month" | "quarter"} period Khoảng thời gian muốn xem
   * @returns {ChartDataPoint[]} Mảng dữ liệu tương thích với biểu đồ
   */
  function computeChartData(activities: any[], period: "week" | "month" | "quarter"): ChartDataPoint[] {
    ...
    for (let i = days - 1; i >= 0; i--) {
      ...
      if (period === "week") {
        ...
        // Nhóm theo quý: hiển thị nhãn theo từng tuần đầu tiên
    ...
    // Tổng hợp các hoạt động vào các nhóm tương ứng
    ...
    // Đối với nhóm theo quý, hãy gộp dữ liệu theo từng tuần
    if (period === "quarter") {
  ...
  /**
   * Xử lý xuât dữ liệu báo cáo tổng hợp ra file CSV
   * @async
   */
  ...
  const reportCards = [
    ...
    { id: "balance" as ReportTab, title: t("balance_report"), desc: t("balance_report_desc"), icon: Calculator, color: "cyan" },
  ...
  // Render báo cáo con (sub-report)
  if (activeReport !== "dashboard") {
    return (
      <div className="space-y-6">
        {/* Khung Header với nút quay lại */}
        ...
        <AnimatePresence mode="wait">
          ...
          >
            ...
            {activeReport === "balance" && <InventoryBalanceReport />}
  ...
  // Chế độ xem bảng điều khiển chính (Dashboard view)
  return (
