...
type CatItem = { id: string; name: string; default_tax_rate?: number };
type UomItem = { id: string; name: string; symbol?: string | null };

/**
 * Component hiển thị và quản lý cả danh mục sản phẩm (Categories) và đơn vị đo lường (UoM).
 */
export default function CategoriesUoMSection() {
  ...
  const [editCatTax, setEditCatTax] = useState(8);
  const [newCatTax, setNewCatTax] = useState(8);
  const [deleteCatId, setDeleteCatId] = useState<string | null>(null);

  // State quản lý đơn vị đo
  ...
  /**
   * Lấy danh sách đơn vị đo lường từ server.
   */
  ...
  async function handleAddCategory() {
    ...
    try {
      const res = await fetch("/api/categories", {
        ...
        body: JSON.stringify({ name: newCat.trim(), default_tax_rate: newCatTax }),
      });
      const data = await res.json();
      if (!res.ok) throw new Error(data.error);
      setNewCat("");
      setNewCatTax(8);
      toast.success(t("cat_added"));
      fetchCategories();
  ...
  async function handleEditCategory(id: string) {
    ...
    try {
      const res = await fetch(`/api/categories/${id}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ name: editCatVal.trim(), default_tax_rate: editCatTax }),
      });
      const data = await res.json();
      if (!res.ok) throw new Error(data.error);
  ...
  /**
   * Xử lý cập nhật tên và ký hiệu của đơn vị đo.
   * @param id ID của đơn vị đo cần cập nhật
   */
  ...
  /**
   * Xử lý xóa một đơn vị đo.
   * @param id ID của đơn vị đo cần xóa
   */
  ...
  return (
    <div className="space-y-6">
          ...
          <div className="flex gap-2">
            ...
            <div className="relative w-20">
              <input
                ...
                step="0.5"
                value={newCatTax}
                onChange={e => setNewCatTax(parseFloat(e.target.value) || 0)}
                className="w-full px-3 py-2.5 bg-neutral-50 dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-700 rounded-xl text-sm font-bold outline-none focus:ring-2 focus:ring-taika-blue text-neutral-900 dark:text-neutral-50 text-center"
              />
                  ...
                  >
                      ...
                      <>
                        ...
                        <div className="relative w-16">
                          <input
                            ...
                            step="0.5"
                            value={editCatTax}
                            onChange={e => setEditCatTax(parseFloat(e.target.value) || 0)}
                            className="w-full bg-white dark:bg-neutral-950 border border-taika-blue rounded-lg px-2 py-1 text-sm font-bold outline-none text-neutral-900 dark:text-neutral-50 text-center"
                          />
                          <span className="absolute right-1.5 top-1/2 -translate-y-1/2 text-[10px] text-neutral-400 pointer-events-none">%</span>
                        </div>
                      </>
                    ) : (
                      <>
                        <span className="flex-1 text-sm font-medium text-neutral-800 dark:text-neutral-200">{cat.name}</span>
                        <span className="text-[11px] font-bold px-2 py-0.5 bg-emerald-100 dark:bg-emerald-500/20 text-emerald-600 dark:text-emerald-400 rounded-md shrink-0">
                          {t("cat_tax_label")}: {cat.default_tax_rate ?? 8}%
                        </span>
                      </>
                    )}
                    ...
                    <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                      {editingCatId === cat.id ? (
                        ...
                        <>
                          <button
                            onClick={() => { setEditingCatId(cat.id); setEditCatVal(cat.name); setEditCatTax(cat.default_tax_rate ?? 8); }}
      ...
      {/* ── Hộp thoại xác nhận xóa đơn vị đo ── */}
      <AnimatePresence>
        {deleteUomId && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
