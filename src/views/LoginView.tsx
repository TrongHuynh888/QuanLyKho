...
// Ô nhập mật khẩu chỉ hiển thị ngắn gọn ký tự vừa gõ (như trên thiết bị di động)
/**
 * Component tạo ô nhập mật khẩu bảo mật hiển thị ngắn ký tự cuối (Password Input).
 */
function PasswordInput({ value, onChange, placeholder, className, required, minLength }: {
  ...
  useEffect(() => {
    ...
    // Hiện ngắn ký tự cuối, ẩn các ký tự còn lại
    if (value.length === 0) {
      setDisplayValue("");
      return;
    }
  ...
  function handleChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    // Tính toán lại xem chuỗi nào vừa bị sửa
...
/**
 * Giao diện Đăng nhập / Đăng ký (Login View).
 * Cung cấp form xác thực bảo mật, khả năng đổi theme (Dark Mode) và ngôn ngữ.
 */
export default function LoginView() {
  ...
  const [isDarkMode, setIsDarkMode] = useState(() => {
    if (typeof window !== "undefined") {
      ...
      return true; // Mặc định là chế độ ban đêm
    }
    return true;
  });
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-blue-50 via-white to-sky-50 dark:from-neutral-950 dark:via-neutral-900 dark:to-neutral-950 relative overflow-hidden transition-colors duration-300">
      ...
      {/* Menu góc phải trên: Đổi giao diện và Đổi ngôn ngữ */}
