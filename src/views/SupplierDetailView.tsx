...
import { ArrowLeft, Edit2, Package, Search, Plus, MapPin, Truck, Calendar, Phone, Mail, FileText, CheckCircle2, Factory, Trash2, ShieldCheck, Tag, Clock, XCircle, Loader2, Globe, CreditCard } from "lucide-react";
...
/**
 * Component hiển thị chi tiết về một nhà cung cấp và lịch sử hoạt động
 *
 * @param {SupplierDetailViewProps} props - Props cho component
 * @param {string} props.supplierId - ID của nhà cung cấp
 * @param {"details" | "history"} [props.defaultSection="details"] - Phần được mở hiển thị ban đầu
 * @param {Function} props.onBack - Event quay lại trang trước
 * @returns {JSX.Element}
 */
export default function SupplierDetailView({ supplierId, defaultSection = "details", onBack }: SupplierDetailViewProps) {
  ...
  useEffect(() => {
    if (data && defaultSection === "history" && historyRef.current) {
      // Sử dụng một timeout ngắn để chắc chắn DOM đã render hoàn tất trước khi cuộn
      setTimeout(() => {
  ...
  /**
   * Truy xuất thông tin chi tiết nhà cung cấp và lịch sử giao hàng
   * @async
   */
            ...
            <div className="space-y-4">
              {(supplier.bank_name || supplier.bank_account) && (
                <div className="flex items-start gap-3">
                  <div className="w-8 h-8 rounded-full bg-blue-50 dark:bg-blue-500/10 flex items-center justify-center text-taika-blue dark:text-blue-400 shrink-0">
                    <CreditCard size={14} />
                  </div>
                  <div>
                    <p className="text-xs text-neutral-400 dark:text-neutral-500 font-medium">Tài khoản ngân hàng</p>
                    <p className="font-bold text-neutral-900 dark:text-neutral-50 text-sm">{supplier.bank_name || "—"}</p>
                    <p className="font-mono text-xs text-neutral-500 dark:text-neutral-400 mt-0.5">{supplier.bank_account || "—"}</p>
                  </div>
                </div>
              )}
              <div className="flex items-start gap-3">
                <Phone size={16} className="text-neutral-400 w-8 mt-1" />
                <div className="flex-1">
                  <p className="text-xs text-neutral-400 dark:text-neutral-500 font-medium">{t("phone_number")}</p>
                  <div className="font-semibold text-neutral-900 dark:text-neutral-50 text-sm space-y-1 mt-0.5">
                    {supplier.phone ? supplier.phone.split(',').map((p, i) => <div key={i}>{p.trim()}</div>) : "—"}
                  </div>
                </div>
              </div>
              ...
              {supplier.website && (
                <div className="flex items-center gap-3">
                  <Globe size={16} className="text-neutral-400 w-8" />
                  <div className="flex-1">
                    <p className="text-xs text-neutral-400 dark:text-neutral-500 font-medium">{t("website", "Website")}</p>
                    <a href={supplier.website} target="_blank" rel="noreferrer" className="font-semibold text-taika-blue hover:underline text-sm break-all">
                      {supplier.website}
                    </a>
                  </div>
                </div>
              ...
              {supplier.notes && (
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1">
                    <p className="text-xs text-neutral-400 dark:text-neutral-500 font-medium">{t("notes", "Ghi chú")}</p>
                    <p className="font-semibold text-neutral-900 dark:text-neutral-50 text-sm">{supplier.notes}</p>
                  </div>
                </div>
              )}
              <div className="flex items-start gap-3">
