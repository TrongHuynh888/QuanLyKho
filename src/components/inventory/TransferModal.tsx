...
/**
 * Cấu trúc thuộc tính cơ sở cho Cửa sổ Điều chuyển cục bộ (Transfer Modal).
 */
...
/**
 * Component Modal Điều hành Lệnh Transfer cục bộ.
 * Người dùng sẽ dùng nó để thao tác "chuyển bao nhiêu kg Hàng này..." từ ô "Này", sang ô "Khác".
 * Tính năng này hỗ trợ việc đồng bộ vị trí trực quan trên Sơ đồ kho map 3D 2D khi nó mở.
 *
 * @param {TransferModalProps & { destLocationFromMap?: LocationWithInventory | null; currentMapWarehouseId?: string; }} props
 * @returns {JSX.Element} Giao diện Modal
 */
export default function TransferModal({
  ...
  // ── Đồng bộ hóa kho đích khi người dùng chọn trên Map ──
  useEffect(() => {
  ...
  // ── Truy xuất các kho hàng hiện có ──
  useEffect(() => {
    fetch("/api/warehouses").then(r => r.json()).then(d => setWarehouses(d || [])).catch(() => {});
  }, []);

  // ── Nạp các vị trí khả dụng khi kho nhận thay đổi ──
  useEffect(() => {
  ...
  // Kiểm tra tương thích ô đích với sản phẩm đang chuyển
  const destMixConflict = (() => {
    if (!selectedItem || !destLocationFromMap) return false;
    const destItems = destLocationFromMap.inventory_items.filter(i => i.quantity > 0);
    if (destItems.length === 0) return false;
    const movingExp = selectedItem.expiry_date ? new Date(selectedItem.expiry_date).toISOString().slice(0, 10) : null;
    return destItems.some(i => {
      if (i.product_id !== selectedItem.product_id) return true;
      const iExp = i.expiry_date ? new Date(i.expiry_date).toISOString().slice(0, 10) : null;
      return iExp !== movingExp;
    });
  })();

  const canSubmit = selectedItem && transferQty > 0 && transferQty <= maxQty && destLocationFromMap && notes.trim().length > 0 && (!destMixConflict || allowMix);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        if (err.error === "BIN_MIX_CONFLICT") {
          toast.error(err.message || "Ô đích không tương thích. Vui lòng bật 'Cho phép trộn lô'.");
          setSaving(false);
          return;
        }
  ...
  return (
    ...
    >
      ...
      <div className="p-4 space-y-5 flex-1 overflow-y-auto">
          ...
          {selectedItem && (
            <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }}>
              ...
              <div className="space-y-2.5">
                ...
                {/* Thanh kéo lượng hàng hóa (Slider) */}
                <input
        ...
        {/* Bước 3: Xác định vị trí nhận (Destination) */}
        <AnimatePresence>
          {selectedItem && transferQty > 0 && (
            <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }}>
              ...
              <div className="space-y-3">
                {/* Thông tin chi tiết vị trí vừa nhận được chọn từ Bản đồ */}
                {destLocationFromMap ? (
                  <div className="rounded-xl border-2 border-purple-400 dark:border-purple-500 bg-purple-50/50 dark:bg-purple-500/5 overflow-hidden">
                    {/* Tiêu đề vị trí đích (Dest header) */}
                    ...
                    {/* Số mặt hàng hiện đang lưu kho tại vị trí đích */}
                    ...
                    {/* Cảnh báo: Vượt sức chứa an toàn */}
                  ...
                  /* Chưa chọn Vị trí nhận - Yêu cầu thao tác trên Bản đồ */
                ...
                {/* C\u1ea3nh b\u00e1o tr\u1ed9n l\u00f4 t\u1ea1i \u00f4 \u0111\u00edch */}
                {destMixConflict && destLocationFromMap && (
                  <div className="p-3 bg-amber-50 dark:bg-amber-500/10 border border-amber-300 dark:border-amber-500/30 rounded-xl space-y-2">
                    <div className="flex items-start gap-2">
                      ...
                      <p className="text-[11px] text-amber-700 dark:text-amber-400 font-medium leading-relaxed">
                        <span className="font-bold">{"\u26a0\ufe0f"} Tr\u1ed9n l\u00f4:</span> \u00d4 \u0111\u00edch \u0111ang ch\u1ee9a s\u1ea3n ph\u1ea9m ho\u1eb7c h\u1ea1n kh\u00e1c v\u1edbi h\u00e0ng b\u1ea1n \u0111ang chuy\u1ec3n. Tr\u1ed9n l\u00f4 g\u00e2y r\u1ee7i ro nh\u1ea7m l\u1eabn ngo\u00e0i th\u1ef1c t\u1ebf.
                      </p>
                    </div>
                    <label className="flex items-center gap-2 cursor-pointer">
                      <input type="checkbox" checked={allowMix} onChange={e => setAllowMix(e.target.checked)}
                        className="w-4 h-4 text-amber-500 border-amber-300 rounded focus:ring-amber-500" />
                      <span className="text-xs font-bold text-amber-700 dark:text-amber-400">Cho ph\u00e9p tr\u1ed9n l\u00f4 v\u00e0o \u00f4 n\u00e0y</span>
                    </label>
                  </div>
                )}
        ...
        {/* Khung tóm tắt thông tin điều chuyển */}
        <AnimatePresence>
