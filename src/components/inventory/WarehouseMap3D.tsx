...
const BIN_GAP = 0.15;
const RACK_GAP = 1.6;   // wider gap between racks
const ZONE_GAP = 3.0;   // wider gap between zones
const FLOOR_Y = 0;
const SHELF_H = 0.03;
const POST_R = 0.04;
...
// ── Rack Frame (wireframe-style shelving unit) ────────────────────────────────
function RackFrame({
  position,
  width,
  depth,
  ...
  // Shelf heights (including bottom and top)
...
// ── Ceiling group (hidden when camera is above) ──────────────────────────────
...
function WarehouseBuilding({ width, depth }: { width: number; depth: number }) {
  ...
  return (
    <group>
      ...
      {/* ── Ceiling elements (only visible from below) ── */}
      <CeilingGroup roofY={roofY}>
        {/* Ceiling beams (cross direction) */}
        {Array.from({ length: numBeamsW }).map((_, i) => {
          const x = -bw / 2 + (i + 0.5) * (bw / numBeamsW);
          return (
        ...
        {Array.from({ length: numBeamsD }).map((_, i) => {
          const z = -bd / 2 + (i + 0.5) * (bd / numBeamsD);
          return (
        ...
        {Array.from({ length: Math.max(2, Math.floor(bw / 6)) }).map((_, i) => {
          const x = -bw / 2 + (i + 0.5) * (bw / Math.max(2, Math.floor(bw / 6)));
          return Array.from({ length: Math.max(2, Math.floor(bd / 8)) }).map((_, j) => {
            const z = -bd / 2 + (j + 0.5) * (bd / Math.max(2, Math.floor(bd / 8)));
            return (
              <group key={`light-${i}-${j}`}>
                <mesh position={[x, roofY - 0.6, z]}>
                  <cylinderGeometry args={[0.01, 0.01, 1.0, 4]} />
                  <meshStandardMaterial color="#555" />
                </mesh>
                <mesh position={[x, roofY - 1.15, z]}>
                  <cylinderGeometry args={[0.25, 0.15, 0.08, 8]} />
                  <meshStandardMaterial color="#9ca3af" metalness={0.8} roughness={0.2} />
                </mesh>
                <pointLight
                  position={[x, roofY - 1.25, z]}
                  intensity={0.6}
                  distance={8}
                  color="#fef3c7"
                  decay={2}
...
function WarehouseScene({
  ...
  return (
    <>
      ...
      <directionalLight position={[0, 6, 15]} intensity={0.2} />
      <hemisphereLight args={["#1e40af", "#0f172a", 0.4]} />
...
// ── Camera reset helper (inside Canvas) ───────────────────────────────────────
