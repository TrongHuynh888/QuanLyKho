...
interface BulkTransferWizardProps {
  ...
  /** Danh sách các vị trí nguồn (chứa hàng) được chọn bởi người dùng */
  selectedSourceLocs: LocationWithInventory[];
  /** Danh sách các vị trí đích (ổ trống) được chọn để chứa hàng */
  selectedDestLocs: LocationWithInventory[];
  /** Danh sách tất cả Kho hàng trong hệ thống */
  warehouses: Warehouse[];
  /** Kho hàng nơi chứa hàng hóa nguồn */
  sourceWarehouse: Warehouse | undefined;
  /** ID của khu vực kho đích (muốn chuyển tới) */
  destWarehouseId: string;
  setDestWarehouseId: (id: string) => void;
  /** Đóng cửa sổ thiết lập */
  onClose: () => void;
  /** Gọi hàm callback sau khi hoàn tất điều chuyển thành công */
  onComplete: () => void;
  /** Callback chuyển vị trí Map sang kho khác */
  onSwitchWarehouse: (id: string) => void;
}

/**
 * Component BulkTransferWizard (Giao diện Chuyển kho hàng loạt)
 * Hỗ trợ giao diện đa bước để người dùng điều hướng lô hàng từ nhiều vị trí này
 * sang nhiều vị trí trống khác, với thuật toán tự phân chia công suất (Capacity).
 *
 * @param {BulkTransferWizardProps} props Các props thiết lập giao diện
 * @returns {JSX.Element} Cửa sổ điều khiển Chuyển hàng loạt
 */
export default function BulkTransferWizard({
  ...
  // Tính tổng khối lượng (kg) của các ô nguồn đã chọn
  const totalSourceKg = useMemo(() => {
    return selectedSourceLocs.reduce((sum, loc) => sum + (loc.total_quantity || 0), 0);
  }, [selectedSourceLocs]);

  // Tính tổng sức chứa (kg) còn lại của các ô đích đã chọn
  ...
  // Ước tính số ô cần thiết để chứa hàng
  ...
  /** Xử lý xác nhận quy trình điều chuyển hàng loạt */
  const handleConfirm = async () => {
    if (!canConfirm) return;
    setSaving(true);
    try {
      const userId = user?.id || null;
      // Khởi tạo danh sách lệnh luân chuyển.
      // Phân bổ hàng hoá vào các ô đích dựa trên sức chứa còn trống.
      const movements: any[] = [];

      // 1. Thu thập tất cả các lô hàng (items) từ ô nguồn
      ...
      // 2. Sắp xếp vị trí đích ưu tiên ô còn sức chứa lớn nhất
      ...
      // 4. Gửi request tạo lệnh luân chuyển lên API
      let successCount = 0;
  ...
  // Xử lý khi nhấn nút "Tiếp tục" -> chuyển sang bước chọn vị trí Nhận
  const handleNext = () => {
    ...
    >
      <div className="mx-4 mb-4 bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-700 rounded-2xl shadow-2xl">
        ...
        <div className="px-5 py-3 flex items-center justify-between border-b border-neutral-100 dark:border-neutral-800">
          ...
          {/* Chỉ báo các bước (Phase indicator) */}
          <div className="flex items-center gap-2">
            <div className={cn(
