...
/**
 * Component Báo cáo Tồn kho (Inventory Report).
 * Hiển thị chi tiết số lượng, giá trị theo từng chi nhánh/kho hàng và xuất báo cáo CSV.
 *
 * @returns {JSX.Element} Giao diện Báo cáo Tồn kho
 */
export default function InventoryReport() {
  ...
  // Làm phẳng mảng dữ liệu trả về từ API
  ...
  const totalQtyByUnit = useMemo(() => {
    return filtered.reduce((acc: Record<string, number>, r) => {
  ...
  const handleExport = () => {
    downloadExcel(
      ...
      [
        ...
        { key: "category", header: t("report_col_category"), width: 25 },
        { key: "warehouse", header: t("report_col_warehouse"), width: 20 },
        { key: "location", header: t("report_col_location"), width: 15 },
        { key: "lot", header: t("report_col_lot"), width: 20 },
        { key: "qty", header: t("report_col_qty"), width: 15 },
        { key: "unit", header: t("report_col_unit"), width: 10 },
        { key: "importPrice", header: t("report_col_import_price"), width: 20 },
        { key: "value", header: t("report_col_value"), width: 20 },
      ],
      reportFilename("inventory_report"),
      "Inventory"
    );
  };
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          ...
          { label: t("report_total_qty"), value: Object.entries(totalQtyByUnit).map(([u, q]) => `${formatVND(q)} ${u}`).join(" | ") || "0", icon: Boxes, color: "green" },
      ...
      <div className="flex flex-col lg:flex-row gap-3 items-start lg:items-center justify-between">
        <div className="flex flex-wrap gap-3 items-center">
          ...
          {/* Ô nhập Tìm vị trí / khu vực cụ thể */}
          <div className="relative">
          ...
          <table className="w-full text-left border-collapse">
            ...
            <tfoot className="bg-neutral-50 dark:bg-neutral-900 border-t-2 border-neutral-200 dark:border-neutral-700">
              {Object.keys(totalQtyByUnit).length === 0 ? (
                ...
                Object.entries(totalQtyByUnit).map(([u, q]: any, i) => (
                  <tr key={u}>
                    {i === 0 ? (
                      <td colSpan={5} className="p-4" rowSpan={Object.keys(totalQtyByUnit).length}>
                    ...
                    <td className="p-4">
                      <span className="text-[15px] font-black font-mono text-taika-blue dark:text-blue-400">{formatVND(q)} <span className="text-[10px] text-taika-blue/50 uppercase">{u}</span></span>
                    </td>
                    {i === 0 ? (
                      <td className="p-4" rowSpan={Object.keys(totalQtyByUnit).length}>
                        <span className="text-[15px] font-bold font-mono text-green-600 dark:text-green-400">{formatVND(totalValue)} ₫</span>
                      </td>
                    ) : null}
                  </tr>
                ))
