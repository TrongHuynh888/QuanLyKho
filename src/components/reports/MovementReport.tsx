...
export default function MovementReport() {
  ...
  // Summary
  const totalInbound = useMemo(() => filtered.filter((r) => r.type === "inbound").reduce((s, r) => s + r.qty, 0), [filtered]);
  const totalOutbound = useMemo(() => filtered.filter((r) => r.type === "outbound").reduce((s, r) => s + r.qty, 0), [filtered]);
  const totalTransfers = useMemo(() => filtered.filter((r) => r.type === "transfer").length, [filtered]);
  ...
  const handleExport = () => {
    const csv = toCSV(
      filtered.map((r) => ({ ...r })),
      [
        { key: "time", header: t("report_col_time") },
        { key: "typeLabel", header: t("report_col_type") },
        { key: "productName", header: t("report_col_product") },
        { key: "sku", header: t("report_col_sku") },
        { key: "qty", header: t("report_col_qty") },
        { key: "batchNumber", header: t("report_col_lot") },
        { key: "warehouse", header: t("report_col_warehouse") },
        { key: "performedBy", header: t("report_col_performed_by") },
        { key: "notes", header: t("report_col_notes") },
      ]
    );
    downloadCSV(csv, reportFilename("movement_report"));
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        {[
          { label: t("report_total_inbound"), value: `${totalInbound.toLocaleString()} kg`, icon: ArrowDownLeft, color: "green" },
          { label: t("report_total_outbound"), value: `${totalOutbound.toLocaleString()} kg`, icon: ArrowUpRight, color: "red" },
          { label: t("report_total_transfers"), value: totalTransfers.toLocaleString(), icon: ArrowRightLeft, color: "blue" },
        ].map((card, i) => (
          <motion.div
            key={i}
